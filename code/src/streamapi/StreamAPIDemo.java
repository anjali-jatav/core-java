...
public class StreamAPIDemo {
	...
	public static List<Student> getTestData() {
		List<Student> list = Arrays.asList(
				new Student(101, "Ambe", 22343, "Assam", 6, 3, LocalDate.of(1993, 11, 23), 9.8, "CSE"),
				...
				new Student(105, "Sumedha", 5675, "Bihar", 4, 2, LocalDate.of(1994, 10, 12), 7.8, "IT"),
...
//	Q1. filter those students whose name starts with 'a'?
		/*
		 * List<Student> filterStudent=new ArrayList<>(); for(Student stu:getTestData())
		 * { if(stu.getStuName().startsWith("a")) { filterStudent.add(stu); } }
		 */
		/*
		 * for(Student stu: filterStudent) { System.out.println(stu); }
		 */
	    List<Student> list1 = getTestData();
		Stream<Student> stream = list1.stream();
		Predicate<Student> p = (s) -> {
			return s.getStuName().startsWith("a");
		};
		Stream<Student> filter = stream.filter(p);
		List<Student> collect = filter.collect(toList());
//collect.forEach(System.out::println);
		getTestData().stream().filter(s -> s.getStuName().startsWith("a")).collect(toList())
...
//Q2. group the students by their state?
		...
		for (Student stu : getTestData()) {
			if (map.containsKey(stu.getStuState())) {
				...
				students1.add(stu);
				map.put(stu.getStuState(), students1);
			}
		}
		/*
	...
	//Q4. group Students by branch?
	Map<String, List<Student>> collect2 = getTestData().stream().collect(groupingBy(s -> s.getStuBranch()));
//    sort the student by cgpa(desc order)
	  students.stream().sorted(Comparator.comparing(Student::getCgpa).reversed()).
